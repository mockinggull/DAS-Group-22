...
```{r}
#| label: Forward-Backward Selection
full_model <- glm(yesno ~ crl.tot+dollar+bang+money+n000+make, data = email, family = binomial(link = "logit"))# Full model (all predictors)
initial_model <- glm(yesno ~ 1, data = email, family = binomial(link = "logit"))  # Null model (only intercept)
# Perform forward-backward selection
stepwise_model <- stepAIC(initial_model, 
                          scope = list(lower = initial_model, upper = full_model), direction = "both")
